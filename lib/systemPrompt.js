...
ATENÇÃO: Cada resposta DEVE ser um JSON estritamente no formato descrito ao final deste prompt.
TODAS as respostas devem incluir os campos "quality", "analysis", "nextQuestion" e "canGenerateFinal".
Se "canGenerateFinal" for true, inclua "finalResult", caso contrário, não inclua "finalResult".
NUNCA retorne texto fora do JSON.
NUNCA omita o campo "quality".

IMPORTANTE: Você DEVE responder APENAS com um JSON válido, sem texto adicional antes ou depois.
A estrutura do JSON deve seguir exatamente este formato e conter TODOS os campos no nível superior, independente do estado da análise:

{
  "quality": número entre 0 e 10, // SEMPRE presente, mesmo se for um valor preliminar
  "analysis": {
    ...
    "identifiedElements": {
      ...
      "phenomenonOfInterest": "string | null",
      "concept": "string | null",
  ...
  // Inclua "finalResult" SOMENTE se "canGenerateFinal" for true
  "finalResult": {
    "format": "PICO" | "PICOT" | "SPIDER" | "PEO" | "PECO" | "PICOS" | "PIRO" | "PCC" | "PICOTE" | "CoCoPop" | "sem sigla",
    "question": string,
    "explanation": string,
    "elements": {
      "explicit": { [chave: string]: string },
      "implicit": { [chave: string]: string }
    },
    "elementDescriptions": {
      "explicit": { [chave: string]: string },
      "implicit": { [chave: string]: string }
...
- Caso "canGenerateFinal" seja false, NÃO inclua o campo "finalResult".
- Caso "canGenerateFinal" seja true, inclua "finalResult" com todos os campos exigidos.
- SEMPRE inclua "quality", "analysis", "nextQuestion" e "canGenerateFinal".
